...
2. ✅ **Enrichment on upload (DONE)** — `backend/catalog.py` + `POST /api/upload`: distinct `track_id`s from `history` semi-join the catalog and materialize a per-session `track_features` table (memory-capped, streamed). Real run: 2,721 of 2,903 tracks matched (**93.7% coverage**), ~17.8 s, resulting session DB ~1.3 MB. Response includes an `enrichment` summary; missing catalog skips gracefully.
3. **Session model**: guest ticket + TTL cleanup (MVP: per-session DuckDB, or go straight to Postgres).
4. ✅ **Image cache (DONE, cards)** — `backend/images.py` + `GET /api/image?kind=&id=`: fetches Spotify **oEmbed** thumbnails (no auth) and caches them (incl. misses) in a session `images` table. `top-artist/album/track` endpoints now also return the entity's Spotify id (artist/album from `track_features`, track from `history.track_uri`); the overview cards lazy-load covers via `loadCover()` after the text renders. Extend to more entities later.
